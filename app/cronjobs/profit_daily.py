...
def reset_volume_f1():
    ...
    if int(date_th) == 0:
        mongo.db.Users.update({}, {'$set': {
            'balance.team_volume_f1': 0,
            'balance.withdraw_usd_week': 0
        }}, multi=True)
...
def profit_daily():
    investment = mongo.db.Investments.find({'$and' :[{'status' : 1},{"status_profit": 1},{"profit_daily": 0},{"new": 0}]} )
    for x in investment:
        ...
        if float(percent) > 0:
            ...
            coin_reward_20 = float(coin_reward)/100
            convert_to_coin = float(commission_usd)/float(price_coin)
...
@bp.route('/jobs-profit-daily-new', methods=['GET'])
def profit_daily_new():
    investment = mongo.db.Investments.find({'$and' :[{'status' : 1},{"status_profit": 1},{"profit_daily": 0},{"new": 1}]} )
    for x in investment:
        amount_usd = int(x['amount_usd'])
        percent = get_profit(amount_usd)
        old_profit_coin = float(x['profit_coin'])
        ...
        if float(percent) > 0:
            user = mongo.db.Users.find_one({'_id' : x['user_id'] })
            commission_usd = round(float(percent)*float(amount_usd)/100, 3)
            commission_50 = float(commission_usd)/2
            ...
            # new_balance_usd = float(user['wallet']['usd_balance']) + float(commission_usd)
            # new_balance_coin_reward = float(user['wallet']['coin_balance_profit'])
            convert_to_coin_satoshi = 0
            # if x['type_invest'] == 50:
            new_balance_usd = float(user['wallet']['usd_balance']) + float(commission_50)
            price_coin = helpers.get_tickers(mongo.db.Tickers, 'coin')
            convert_to_coin = float(commission_50)/float(price_coin)
            convert_to_coin_satoshi = round(float(convert_to_coin)*100000000,0)
            new_balance_coin_reward = float(user['wallet']['coin_balance']) + float(convert_to_coin_satoshi)
            ...
            mongo.db.Users.update({'_id': user['_id']},{"$set":
                {
                    # wallet_update: round(new_balance_coin, 0),
                    "wallet.usd_balance": float(round(new_balance_usd,4)),
