...
def investInvoice():
    try:
        ...
        if form.validate_on_submit():
            ...
            #     "msg": "Coming Soon"
            # })
            payment = request.form['method']
            method_payment = request.form['method']
            # if payment =="ETH":
            #     return jsonify({
            #         "success": 0,
            #         "msg": "Coming Soon"
            ...
            currency = request.form['method']
            currency_price = payment
            if payment == "TRX1":
                currency_price = "TRX"
            elif payment == "BTC1":
                currency_price = "BTC"
                ...
                currency_price = payment
            price = helpers.get_tickers(mongo.db.Tickers, currency_price.lower())
            usd_fee = float(amount)*0.01
            amount_coin_fee =round(usd_fee/float(price), 8)
            amount_coin_invest = round(float(amount)/float(price),8)
            amount_coin_invest_satoshi = float(amount_coin_invest)*100000000
            amount_coin_invest_50 = float(amount_coin_invest_satoshi)/2
            amount_50 = float(amount)/2
            price_qtc = helpers.get_tickers(mongo.db.Tickers, 'coin')
            amount_qtc = float(amount_50)/float(price_qtc)
            amount_qtc_satoshi = float(amount_qtc)*100000000
            ...
            total_coin_amount_send = float(total_coin_amount)
            # total_coin_amount = 0.005
            balance = 0
            payment1 = ["QTC1", "BTC1", "ETH1", "TRX1", "USDT1"]
            if payment == "QTC" or payment == "QTC1":
                crypto_address = user['wallet']['qtc_address']
                address = user['wallet']['qtc_address']
                balance = user['wallet']['coin_balance']
            elif payment =="BTC" or payment == "BTC1":
            ...
            elif payment =="ETH" or payment == "ETH1":
            ...
            elif payment =="TRX" or payment == "TRX1":
            ...
            elif payment =="USDT" or payment == "USDT1":
            ...
            # Method 2 ===================================================
            type_invest = 100
            if method_payment in payment1:
                amount_invest = float(amount)
                amount_qtc_satoshi = 0
                amount_coin_invest_50= amount_coin_invest_satoshi
                amount_50=0
                if float(amount_coin_invest_satoshi) > float(balance):
                    return jsonify({
                        "success": 0,
                        "msg": "Your %s balance is not enough "% (request.form['payment'])
                    })
            else:
                type_invest = 50
                amount_invest = float(amount)/2
                print(amount_qtc_satoshi, amount_coin_invest_50)
                if float(amount_qtc_satoshi) > float(user['wallet']['coin_balance']):
                    return jsonify({
                        "success": 0,
                        "msg": "Your QTC balance is not enough "
                    })
                if float(amount_coin_invest_50) > float(balance):
                    return jsonify({
                        "success": 0,
                        "msg": "Your %s balance is not enough "% (payment)
                    })
            
            new_qtc_balance = float(user['wallet']['coin_balance'])-float(amount_qtc_satoshi)
            new_payment_balance = float(balance)-float(amount_coin_invest_50)
            mongo.db.Users.update({ "_id" : session['user_id'] }, { '$set': { 
                "wallet.coin_balance": round(new_qtc_balance, 0),
                wallet_update: round(new_payment_balance, 0),
            }})
            ...
            amount_coin_satoshi_i = amount_coin_invest_50
            dataInvest = {
                '_id': _id,
                ...
                "type_invest": type_invest,
                "new": 1
            }
            investments.insert(dataInvest)
            total_invest = float(user['balance']['total_invest']) + float(amount)
            commission_remain = float(amount) * 3
            ...
            usd_reward = float(amount)*float(percent_coin_reward) / 100
            ...
            mongo.db.Users.update({'_id':user['_id']},{'$set':{
                        ...
                        # 'wallet.coin_reward_date': datetime.now() + timedelta(days=30),
                        'level': level,
                        'balance.max_out_day': float(amount),
                        'balance.commission_remain': total_commission_remain,
                        'balance.total_invest': total_invest
            }})
            calTeamVolume(user['_id'], amount)
            binaryAmount(user['_id'], amount_invest)
            calCommissionLevelBonus(user['_id'], amount_invest, user['name'])
            if user['name'] != "root":
                amount_bot = round(float(amount), 0)
                text_bot = str(user['name']) + ' - $'  + str(amount) + ' = ' + str(amount_bot) + ' ' + str(payment)
        ...
        # Invoice = {'error': 'ok', 'result': {'amount': '0.04829857', 'txn_id': 'CPED1H5KTBE5ZXIHFFDIFQ4TZU', 'address': '3H6uNKrUbHDgTG2DRbyCGVxpmw9cP6L4A2', 'confirms_needed': '2', 'timeout': 27000, 'checkout_url': 'https://www.coinpayments.net/index.php?cmd=checkout&id=CPED1H5KTBE5ZXIHFFDIFQ4TZU&key=56c6625cdc141ac59991116c57281f81', 'status_url': 'https://www.coinpayments.net/index.php?cmd=status&id=CPED1H5KTBE5ZXIHFFDIFQ4TZU&key=56c6625cdc141ac59991116c57281f81', 'qrcode_url': 'https://www.coinpayments.net/qrgen.php?id=CPED1H5KTBE5ZXIHFFDIFQ4TZU&key=56c6625cdc141ac59991116c57281f81'}}
                ...
                return jsonify({
                    ...
                    "amount": round(total_coin_amount_send, 0) if currency == "TRX" or currency == "USDT" else round(total_coin_amount_send, 8),
                    "amount_usd": amount,
                    "currency": currency
                })
            else:
...
def CheckStatus(currency):
    ...
    price_trx = helpers.get_tickers(mongo.db.Tickers, currency.lower())
    date_invoice = data['createdAt'] + timedelta(minutes=45)
    date_now = datetime.now()
    amount_send = float(data['amount_coin_invest'])
    if date_now > date_invoice:
        new_coin = float(data['amount_usd'])/float(price_trx)
        amount_satoshi = float(new_coin)*100000000
        mongo.db.Invoices.update({'_id': data['_id']}, {'$set':{
            'total_coin_amount': float(round(amount_satoshi, 0)),
            'amount_coin_invest': round(new_coin,8),
            'createdAt': datetime.now()
        }})
    ...
    if data is None:
        ...
        return jsonify({"success": 0, "amount_send": amount_send})
        ...
        if customer_ml_p_node is None:
            ...
            if percent != 0 and customer_ml_p_node['level'] != 0:
                ...
                if float(commission) > 0:
                    commission_50 = float(commission)/2
                    ...
                    price_coin = helpers.get_tickers(mongo.db.Tickers, 'coin')
                    convert_to_coin = float(commission_50)/float(price_coin)
                    ...
                    balance_usd = round(float(customer_ml_p_node['wallet']['usd_balance']) + float(commission_50), 2)
                    users.update({'_id': customer_ml_p_node['_id']},{'$set': {
                        "wallet.coin_balance": round(new_balance_coin_reward, 0),
                        'wallet.usd_balance': balance_usd,
